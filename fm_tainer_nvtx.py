...
@nvtx.annotate("prepare_dataset", color="cyan")
...
@nvtx.annotate("main", color="white")
def main(cfg: DictConfig) -> None:
    ...
    with nvtx.annotate("model_tokenizer_creation", color="lightgreen"):
        model, tokenizer, model_config = create_dit_model_and_tokenizer(cfg)

    # 5. Create Flow Matching Scheduler
    with nvtx.annotate("scheduler_creation", color="gold"):
        scheduler = create_flow_matching_scheduler(cfg, model_config)

    # 6. Prepare Datasets
    with nvtx.annotate("dataset_preparation", color="lightblue"):
    ...
    with nvtx.annotate("trainer_creation", color="pink"):
        trainer = FMTrainer(
            model=model,
            args=training_args,
    ...
    with nvtx.annotate("training_loop", color="red"):
